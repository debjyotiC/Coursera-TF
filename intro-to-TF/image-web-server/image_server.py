...
def write_to_csv(num, clf):
    values = {'Image': [num], 'classified': [clf]}
    df_w = pd.DataFrame(values, columns=['Image', 'classified'])
    df_w.to_csv("/home/pi/Coursera-TF/intro-to-TF/image-web-server/results.csv", index=None, header=True)
...
def image_classify():
    if request.method == 'POST':
        ...
        load_model = tf.lite.Interpreter(
            model_path="/home/pi/Coursera-TF/intro-to-TF/image-web-server/saved_model/tflite_model/converted_model.tflite")
        load_model.allocate_tensors()
        input_details = load_model.get_input_details()
        output_details = load_model.get_output_details()
        img = image.load_img('/home/pi/Coursera-TF/intro-to-TF/image-web-server/static/uploaded_images/' + from_path,
                             target_size=(300, 300))
